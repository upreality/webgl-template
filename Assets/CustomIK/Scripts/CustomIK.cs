...
public class CustomIK : MonoBehaviour {
	...
	{
		if (!isEnabled) return;
		if (rootBone == null || elbowBone == null || endBone == null || targetIK == null || elbowIK == null) return;
		CalculateIK ();
	}
	...
	void CalculateIK(){
		...
		var multiplier = inverted ? -1 : 1;
		var lookAtPos = inverted ? 2 * Handler.position - targetIK.position : targetIK.position;
		if (targetDistance < armLength && targetDistance > minimumLength) {
            rootBone.localEulerAngles = new Vector3(- angleUpperArm, 0, 0) * multiplier;
            elbowBone.localEulerAngles = new Vector3(180f - angleElbow, 0, 0) * multiplier;
            Handler.LookAt(lookAtPos, elbowIK.position - elbowsAdjacent);
		}else if(targetDistance < minimumLength){
            rootBone.localEulerAngles = Vector3.zero;
            Handler.LookAt(lookAtPos, elbowIK.position - elbowsAdjacent);
            elbowBone.localEulerAngles = new Vector3(180f, 0, 0) * multiplier;
		}else{
            rootBone.localEulerAngles = Vector3.zero;
            Handler.LookAt(lookAtPos, elbowIK.position - elbowsAdjacent);
            elbowBone.localEulerAngles = Vector3.zero;
		}
		#endregion
